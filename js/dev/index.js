import { d as dataMediaQueries, s as slideToggle, a as slideUp, b as bodyLockToggle, c as bodyLockStatus, g as getDigFormat, e as gotoBlock, u as uniqArray, f as bodyUnlock, h as getHash } from "./common.min.js";
(function polyfill() {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!platforms) return;
  const wrapper = platforms.querySelector(".platforms__wrapper");
  const toggle = platforms.querySelector(".platforms__toggle");
  toggle.addEventListener("click", () => {
    wrapper.classList.toggle("_open");
...
const allLangs = ["UA", "EN", "ES"];
function renderLangList(container, currentLang) {
  const list = container.querySelector("[data-lang-list]");
  if (!list) return;
  list.innerHTML = "";
  allLangs.filter((lang) => lang !== currentLang).forEach((lang) => {
    const li = document.createElement("li");
    li.innerHTML = `<button type="button" data-lang-option="${lang}">${lang}</button>`;
    list.appendChild(li);
  });
}
document.addEventListener("click", function(e) {
  ...
  if (switcher) {
    const currentLangEl = switcher.querySelector(".lang-switch__text");
    if (!currentLangEl) return;
    const currentLang = currentLangEl.textContent.trim();
    renderLangList(switcher, currentLang);
    switcher.classList.toggle("open");
    return;
  }
  document.querySelectorAll("[data-lang-switch].open").forEach((el) => el.classList.remove("open"));
});
document.addEventListener("click", function(e) {
  const btn = e.target.closest("[data-lang-option]");
  if (!btn) return;
  const container = document.querySelector("[data-lang-switch]");
  console.log(container);
  if (!container) return;
  const current = container.querySelector(".lang-switch__text");
  if (!current) return;
  current.textContent = btn.dataset.langOption;
...
function initMarquee() {
  let isInit = false;
  function checkWidth() {
    if (window.innerWidth > 768 && !isInit) {
      marquee();
      isInit = true;
    }
    if (window.innerWidth <= 768 && isInit) {
